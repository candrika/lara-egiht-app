  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <form method="post" action="/update_pemasukan/{{$in->list_kategori_id}}">
            {{ csrf_field() }}
            {{method_field('PUT')}}
            <input type="hidden" value="{{$in->list_kategori_id}}"/>
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Nama Kategori</label>
              <div class="col-sm-6">
                <input type="text" class="form-control" required="required" value="{{$in->nama_kategori}}" name="nama_kategori">
              </div>
            </div>
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Deskripsi</label>
              <div class="col-sm-6">
                <input type="text" class="form-control" required="required" value="{{$in->deskripsi}}" name="deskripsi">
              </div>
            </div>
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Jenis Kategori</label>
              <div class="col-sm-6">
                <select class="form-select" aria-label="Default select example" value=="{{$in->jenis_kategori}}" name="jenis_kategori">
