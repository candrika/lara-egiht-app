  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <form method="post" action="/update/transaksi/{{$trx->transaksi_id}}">
            {{ csrf_field() }}
            {{method_field('PUT')}}
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Jenis Kategori</label>
              <div class="col-sm-5">
                <select class="form-select" aria-label="Default select example" name="jenis_kategori">
                  <option value="{{$trx->list_kategori_id}}" seleteced>{{$trx->nama_kategori}}</option>
                  @foreach($list as $ls)
                  <option value="{{$ls->list_kategori_id}}">{{$ls->nama_kategori}}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Nama Transaksi</label>
              <div class="col-sm-5">
                <select class="form-select" aria-label="Default select example" name="jenis_transaksi">
                  <option value="{{$trx->jenis_transaksi_id}}" seleteced>{{$trx->jenis_transaksi}}</option>
            ...
            <div class="mb-3 row">
              ...
              <div class="col-sm-5">
                <input type="text" class="form-control" value="{{$trx->nominal}}" name="nominal" autocomplate="false" style="text-align:right">
              </div>
            </div>
            <div class="mb-3 row">
              <label for="staticEmail" class="col-sm-2 col-form-label">Deskripsi</label>
              <div class="col-sm-5">
                <textarea class="form-control" autocomplate="false" name="deskripsi">{{$trx->deskripsi}}</textarea>
              </div>
            </div>
            <div class="mb-3 row">
